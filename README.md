...
The implementation, training pipeline, checkpoint recovery, command-line predictor, evaluation harness, monitoring dashboard, text preprocessor, and Streamlit application are operational. The trained checkpoint at `runs/main_run/checkpoints/latest.pt` is verified at step `100000` and is the default inference artifact.

The standalone PFN is not yet reliable enough for production on mixed real-world categorical data. It performs well on several numeric benchmarks but can collapse to the majority class on the supplied churn and loan examples. The Streamlit app therefore defaults to **Auto reliable**, which compares the frozen PFN with a supervised random-forest baseline using a validation split from the support data. That mode is a practical fallback, not a claim of zero-shot PFN performance.
...
| Dataset | PFN | Dummy | Logistic regression | Random forest | HistGradientBoosting |
| --- | ---: | ---: | ---: | ---: | ---: |
| Iris | 88.9% | 33.3% | 91.1% | 88.9% | 86.7% |
| Wine | 88.9% | 38.9% | 98.1% | 100.0% | 100.0% |
| Breast Cancer | 90.6% | 62.6% | 97.7% | 94.7% | 95.9% |
| Digits | 35.0% | 10.2% | 92.2% | 93.5% | 93.9% |
| Titanic | 63.4% | 61.8% | 81.4% | 77.6% | 81.4% |
...
- `eod_report_aug29.md`: end-of-day record with the checkpoint, GUI, leakage, and validation findings.
...
- `reports/evaluation_results.md`: benchmark table generated by the evaluator.
